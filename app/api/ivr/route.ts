...
// תקנון מזהה טלפון לפורמט ספרות אחיד
...
async function handleRequest(req: Request) {
  try {
    ...
    // === מקרה A: המשתמש מקיש PIN חדש במפורש (או שאין לו סשן) ===
    ...
    if (activePin) {
      // בדיקה שהמשחק אכן קיים ב-game_players
      ...
      // אם המשחק נמחק על ידי המנחה בסיום
      if (!activePlayer) {
      ...
      // אם כבר מחובר ולא הקיש תשובה חדשה
